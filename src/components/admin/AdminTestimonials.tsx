...
import { Plus, Pencil, Trash2, X, Check, Loader2, AlertTriangle, Star } from 'lucide-react';
...
export default function AdminTestimonials() {
  ...
  const openEdit = (t: Testimonial) => { setForm({ ...t }); setModalOpen(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      showToast(form.id ? 'Testimonial updated!' : 'Testimonial created!', 'success');
      closeModal();
      fetchTestimonials();
    }
    setSaving(false);
    ...
    <div className="min-h-screen bg-black text-white p-6 md:p-8">
      ...
      >
        <div>
          ...
          <p className="text-gray-400 text-sm">Manage client reviews and testimonials.</p>
        </div>
        <button
          onClick={openAdd}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  {['Name', 'Role / Company', 'Rating', 'Published', 'Order', 'Actions'].map((h) => (
              ...
              <tbody>
                {testimonials.map((t) => (
                  <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-3.5">{renderStars(t.rating)}</td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${t.published ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/5 text-gray-500 border border-white/10'}`}>
                        {t.published ? 'Yes' : 'No'}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-gray-400">{t.sort_order}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(t)} className="p-1.5 rounded-lg text-gray-400 hover:text-cyan-400 hover:bg-cyan-400/10 transition-all">
                          <Pencil size={15} />
                        </button>
                        {deleteConfirm === t.id ? (
                          <div className="flex items-center gap-1">
                            <button onClick={() => handleDelete(t.id)} disabled={deleting === t.id} className="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-xs hover:bg-red-500/30 transition-all">
                              {deleting === t.id ? <Loader2 size={12} className="animate-spin" /> : 'Confirm'}
                            </button>
                          ...
                          <button onClick={() => setDeleteConfirm(t.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-400/10 transition-all">
                            <Trash2 size={15} />
                          </button>
                        )}
                      </div>
      ...
      {/* Modal */}
      <Modal isOpen={modalOpen} onClose={closeModal} title={form.id ? 'Edit Testimonial' : 'Add Testimonial'}>
